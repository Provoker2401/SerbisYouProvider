...
const ApplicationForm3 = () => {
  ...
  const petGroomingItems = [
    {
      ...
      items: [
        ...
        { name: "Scent Gland Cleaning (RABBIT)", id: 1032 },
  ...
  const handleToggleSelectAll = (category, isSelected) => {
    const categoryItems = category.items.map((item) => item.id);
    if (isSelected) {
      setSelectedItems((prevSelectedItems) =>
        prevSelectedItems.filter((id) => !categoryItems.includes(id))
      );
    } else {
      setSelectedItems((prevSelectedItems) =>
        [...new Set([...prevSelectedItems, ...categoryItems])]
      );
    }
  };
  ...
  const renderCustomHeader = () => {
    ...
    return (
      <View style={styles.headerContainer}>
        ...
        <Pressable
          onPress={() => handleToggleSelectAll(category, isSelected)}
        >
